...
public class FireLawn_ELECTRONIC_010 extends LoginClass {
	@Test
	public void fireLawn_electronics_008() {
		...
		if(size>0) {
			for(WebElement actualPrice:camera.getActualPriceOfAllProduct()) {
				if(actualPrice.isDisplayed()) {
			    test.log(LogStatus.PASS, test.addScreenCapture( getElementPhoto(actualPrice)));	
				}
				else {
