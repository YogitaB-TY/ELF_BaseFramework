...
public class FireLawn_ELECTRONIC_007 extends LoginClass {
	
	@Test
	
	public void fireLawn_electronics_007() {
		...
		if(size>0) {
			for(WebElement productImage:camera.getAllCameraImages()) {
				if(productImage.isDisplayed()) {
			    test.log(LogStatus.PASS, test.addScreenCapture( getElementPhoto(productImage)));	
				}
				else {
